...
# read in validation labels
validation_labels = []
for i in ["technology", "food", "education", "geography"]:
    labels = pd.read_csv("dummy_labels/dummy_labels_" + i + ".csv")
    validation_labels.extend(labels["label"].tolist())

VALIDATION_ARTICLES = {
    normalize(a) for a in validation_labels
}
